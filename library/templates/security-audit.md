...
## Model Note
On Claude Fable 5, security-adjacent work (vulnerability analysis, exploit discussion, control-weakness evidence) sees elevated safety-classifier false positives across all three roles — requests can return `stop_reason: "refusal"` even within the authorized engagement. Configure the team to fall back to Claude Opus 4.8 (Anthropic's documented fallback) and keep prompts scoped tightly to the Audit Scope Document to reduce classifier triggers.
